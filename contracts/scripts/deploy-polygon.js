...
async function main() {
  console.log("🚀 Deploying TokenFactory and TokenImplementation to Polygon Mainnet");
  ...
  if (Number(network.chainId) !== 137) {
    console.error("❌ ERROR: This script is designed for Polygon mainnet (Chain ID: 137)");
  ...
  if (balance < ethers.parseEther("0.1")) {
    ...
    console.error("Please ensure you have at least 0.1 MATIC for gas fees");
    process.exit(1);
  }
  ...
  try {
    ...
    // Test service fees
    const basicFee = await tokenFactory.getServiceFee(137, 0); // Basic plan
    const professionalFee = await tokenFactory.getServiceFee(137, 1); // Professional plan
    const enterpriseFee = await tokenFactory.getServiceFee(137, 2); // Enterprise plan
    
    console.log("   Basic Plan Fee:", ethers.formatEther(basicFee), "MATIC");
    console.log("   Professional Plan Fee:", ethers.formatEther(professionalFee), "MATIC");
    console.log("   Enterprise Plan Fee:", ethers.formatEther(enterpriseFee), "MATIC");
    ...
    // Save to file
    const fs = require("fs");
    const deploymentPath = "./deployments/polygon-deployment.json";
    ...
    console.log("\n🔗 Polygonscan Links:");
    console.log("-".repeat(40));
    console.log("TokenImplementation:", `https://polygonscan.com/address/${implementationAddress}`);
    console.log("TokenFactory:", `https://polygonscan.com/address/${factoryAddress}`);
