...
async function main() {
  console.log("🚀 Deploying TokenFactory and TokenImplementation to Base Mainnet");
  ...
  if (Number(network.chainId) !== 8453) {
    console.error("❌ ERROR: This script is designed for Base mainnet (Chain ID: 8453)");
  ...
  try {
    ...
    // Test service fees
    const basicFee = await tokenFactory.getServiceFee(8453, 0); // Basic plan
    const professionalFee = await tokenFactory.getServiceFee(8453, 1); // Professional plan
    const enterpriseFee = await tokenFactory.getServiceFee(8453, 2); // Enterprise plan
    ...
    console.log("\n🔗 Basescan Links:");
    console.log("-".repeat(40));
    console.log("TokenImplementation:", `https://basescan.org/address/${implementationAddress}`);
    console.log("TokenFactory:", `https://basescan.org/address/${factoryAddress}`);
