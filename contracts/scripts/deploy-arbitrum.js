...
async function main() {
  console.log("🚀 Deploying TokenFactory and TokenImplementation to Arbitrum One Mainnet");
  ...
  if (Number(network.chainId) !== 42161) {
    console.error("❌ ERROR: This script is designed for Arbitrum One mainnet (Chain ID: 42161)");
  ...
  if (balance < ethers.parseEther("0.01")) {
    ...
    console.error("Please ensure you have at least 0.01 ETH for gas fees");
    process.exit(1);
  }
  ...
  try {
    ...
    // Test service fees
    const basicFee = await tokenFactory.getServiceFee(42161, 0); // Basic plan
    const professionalFee = await tokenFactory.getServiceFee(42161, 1); // Professional plan
    const enterpriseFee = await tokenFactory.getServiceFee(42161, 2); // Enterprise plan
    ...
    // Save to file
    const fs = require("fs");
    const deploymentPath = "./deployments/arbitrum-deployment.json";
    ...
    console.log("✅ Network: Arbitrum One Mainnet (Chain ID: 42161)");
    console.log("✅ Deployer:", deployer.address);
    ...
    console.log("\n🔗 Arbiscan Links:");
    console.log("-".repeat(40));
    console.log("TokenImplementation:", `https://arbiscan.io/address/${implementationAddress}`);
    console.log("TokenFactory:", `https://arbiscan.io/address/${factoryAddress}`);
