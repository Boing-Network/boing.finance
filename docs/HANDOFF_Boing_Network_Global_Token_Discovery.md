...
## 9. `boing.finance` integration checklist (this repo)

| Area | Status |
|------|--------|
| **Dependency** | Frontend uses **`file:../boing.network/boing-sdk`** in CI/local; that checkout must be **≥ 0.3.0** (or publish **`boing-sdk@^0.3.0`** when moving off the file link). |
| **L1 token directory** | **`BoingNativeDexIntegrationContext`** loads paginated **`boing_listDexTokens`** via **`collectAllDexTokensPagesL1`** (`light: true`), merges after indexer rows so **L1 overwrites** indexer labels for the same `id`. |
| **L1 pool directory** | When **`boing_listDexPools`** is available, extra pools are merged into **`venues`** (then **`hydrateCpPoolVenuesFromRpc`**) after register-log merge; disable with **`REACT_APP_BOING_NATIVE_DEX_DISCOVERY_POOLS=0`**. |
| **RPC capability** | **`resolveNativeDexL1DiscoveryCapabilities`** unions **`boing_rpcSupportedMethods`** with **`boing_getRpcMethodCatalog`** names; probes only when **both** are empty. Optional **`preflightRpc`** via **`REACT_APP_BOING_NATIVE_DEX_DISCOVERY_PREFLIGHT=1`** (metadata in context / Developer tools). |
| **Leg decimals** | **`tokenADecimals` / `tokenBDecimals`** from L1 pool pages attach to **`venues`**; swap + pools table use **`formatNativePoolReserveDisplay`** when decimals are known (venue or indexer / L1 token row). |
| **Pagination** | Cursors are **opaque**; max pages from **`REACT_APP_BOING_NATIVE_DEX_DISCOVERY_MAX_PAGES`** (default **100**, hard cap **250**). |
| **Spam / dust** | Optional **`REACT_APP_BOING_NATIVE_DEX_DISCOVERY_MIN_LIQUIDITY_WEI`** / **`MIN_RESERVE_PRODUCT`** forwarded to the node; discovery is **not endorsement** (see §5). |
| **Implementation detail** | **`frontend/src/services/nativeDexL1Discovery.js`** — see also [INTEGRATION_NATIVE_DEX_L1_DISCOVERY.md](./INTEGRATION_NATIVE_DEX_L1_DISCOVERY.md). |

Upstream operator / multi-repo checklist: [Boing Network — HANDOFF-DEPENDENT-PROJECTS.md](https://github.com/Boing-Network/boing.network/blob/main/docs/HANDOFF-DEPENDENT-PROJECTS.md).
