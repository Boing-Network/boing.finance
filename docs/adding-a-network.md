...
- **dexFactory / dexRouter / weth:** Required for **Swap (Boing DEX)**. **Create Pool** and **Liquidity** (add/remove on an existing pair) also work on Uniswap V2–compatible venues listed in `frontend/src/config/uniswapV2Compat.js` (PancakeSwap V2 on BNB). Ethereum is not in the Boing DEX rollout; see [contracts.md](./contracts.md).
...
- **Liquidity** add/remove on an existing pair uses the same venue as Create Pool (Boing router, else Uniswap/Pancake V2). On Boing L1 it uses native `add_liquidity` / `remove_liquidity`.
...
- `featureSupport.js` (reads from `contracts.js`; Create Pool / Liquidity also read `uniswapV2Compat.js`)
