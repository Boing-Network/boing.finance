...
3. **GitHub Actions** — Workflow [.github/workflows/native-dex-indexer.yml](../.github/workflows/native-dex-indexer.yml) runs on a schedule, caches state, writes `native-dex-indexer-stats.json` via `NATIVE_DEX_INDEXER_OUT_PATH`, uploads artifact. Repo variables: `BOING_INDEXER_RPC_URL`, `NATIVE_DEX_INDEXER_REGISTER_FROM_BLOCK`, `NATIVE_DEX_INDEXER_LOG_SCAN_BLOCKS`, optional `NATIVE_DEX_INDEXER_TOKEN_USD_JSON`, `NATIVE_DEX_INDEXER_TOKEN_DIRECTORY_JSON` (keep JSON compact; vars size limits apply). **Optional R2 upload:** set `NATIVE_DEX_INDEXER_R2_BUCKET` and `NATIVE_DEX_INDEXER_R2_OBJECT_KEY` (e.g. `stats/native-dex-indexer-stats.json`); job uses `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`. **Deploy workflow** [.github/workflows/deploy-frontend.yml](../.github/workflows/deploy-frontend.yml): optional `REACT_APP_BOING_NATIVE_DEX_INDEXER_STATS_URL` (production) and `REACT_APP_BOING_NATIVE_DEX_INDEXER_STATS_URL_STAGING` bake the indexer URL into the bundle; `wrangler pages deploy` from `frontend/` applies `wrangler.toml` bindings (KV for `/api/native-dex-indexer-stats`).
